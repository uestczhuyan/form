...
//		sum = sum - baseNotify * MapInitUtil.getValueInPox(oldpy, oldpx, map);
		sumList.add((-1)*baseNotify * MapInitUtil.getValueInPox(nowPoint, map));
	...
	public  boolean doRight(boolean show,Context context) {
		...
		if (list == null || list.size() <= 0) {
			if(show){
				Toast.makeText(context, "数据出错：py=" + py,
...
//		sum = sum + baseNotify * MapInitUtil.getValueInPox(oldpy, oldpx, map);
	...
	public boolean moveBack(){
		if (results.size() > 0) {
			System.out.println(results.size()+"  "+sumList.size()+"  "+points.size());
	...
	/**
	 * @param baseNotify the baseNotify to set
	 */
	...
	/**
	 * @param nowPoint the nowPoint to set
	 */
